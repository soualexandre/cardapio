...
const defaultStyle = {
  ...
  tabActive: 'bg-gray-600 text-white shadow-gray-200',
  tabInactive: 'bg-gray-50 text-gray-800 hover:bg-gray-100',
  badgeColor: 'bg-gray-600',
  badgeText: 'text-white',
  sectionBorder: 'border-gray-200',
}

export default function CardapioClient({ categorias: initialCategorias }: { categorias: Categoria[] }) {
  const [categorias] = useState<Categoria[]>(initialCategorias)
  const [categoriaAtiva, setCategoriaAtiva] = useState<string>(initialCategorias[0]?.id ?? '')
  const [menuAberto, setMenuAberto] = useState<boolean>(false)
  const [scrolled, setScrolled] = useState(false)
  const [categoriasExpandidas, setCategoriasExpandidas] = useState<Record<string, boolean>>(
    Object.fromEntries(initialCategorias.map(c => [c.id, true]))
  )
    ...
    <div className="min-h-screen" style={{ background: '#FFFBEB' }}>
      ...
      >
        ...
        <div className="relative container mx-auto px-4 py-10 text-center max-w-2xl">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <div className="glass-pill rounded-2xl px-5 py-3 text-center min-w-[120px]">
              ...
              <p className="text-white text-xs font-semibold leading-tight">Tatersal<br />Sindicato Rural</p>
            </div>
            <div className="glass-pill rounded-2xl px-5 py-3 text-center min-w-[120px]">
        ...
        <div className="relative h-8 overflow-hidden mt-2">
          <svg viewBox="0 0 1440 40" fill="none" xmlns="http://www.w3.org/2000/svg" className="absolute bottom-0 w-full" preserveAspectRatio="none">
            <path d="M0 40 C360 0 1080 0 1440 40 L1440 40 L0 40Z" fill="#FFFBEB" />
          </svg>
        </div>
      </section>
      ...
      >
        <div className="overflow-x-auto hide-scrollbar px-3 py-3">
          <div className="flex gap-2 min-w-max md:min-w-0 md:justify-center">
            {categorias.map(cat => {
              const s = categoryStyle[cat.id] ?? defaultStyle
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap shadow-sm ${
                    isActive ? `${s.tabActive} shadow-md scale-105` : s.tabInactive
                  }`}
                >
      ...
      <main id="cardapio-anchor" className="container mx-auto px-3 py-6 max-w-5xl">
        {categorias.map(categoria => {
          const s = categoryStyle[categoria.id] ?? defaultStyle
          const expanded = categoriasExpandidas[categoria.id] !== false

          return (
            <section key={categoria.id} id={categoria.id} className="mb-8 scroll-mt-36 category-section">
              <button
                    ...
                    >
                      <div className="relative h-36 overflow-hidden bg-amber-50">
                        ...
                        <div className="absolute top-2 right-2">
                          <span className={`${s.badgeColor} ${s.badgeText} text-xs font-extrabold px-2.5 py-1 rounded-full shadow-lg`}>
                            {item.preco}
                          </span>
                        </div>
                      </div>
      ...
      {/* ── FOOTER ───────────────────────────────────────────── */}
      <footer className="text-white py-10 px-4" style={{ background: 'linear-gradient(135deg, #7C2D12 0%, #92400E 40%, #B45309 100%)' }}>
        <div className="container mx-auto max-w-2xl text-center">
          ...
          <div className="flex justify-center gap-4 mb-8">
            {[
              ...
              { icon: <Facebook size={20} className="text-amber-100" />, label: 'Facebook' },
              { icon: <Phone size={20} className="text-amber-100" />, label: 'Telefone' },
            ].map(s => (
              <a
                key={s.label}
                href="#"
          ...
          <div className="border-t pt-5 text-xs" style={{ borderColor: 'rgba(180, 83, 9, 0.5)', color: 'rgba(253, 230, 138, 0.6)' }}>
